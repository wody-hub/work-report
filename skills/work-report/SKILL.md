...
- **로그 포맷이 버전마다 다르다.** Codex 는 구형(2025년, `session_meta` 없음)과 신형이 공존하고 전환기에는 섞여 있다. 필드 위치와 새 포맷 대응 절차는 저장소의 `docs/log-formats.md` 에 있다. 커버리지에 `codex_메타없음` 이나 `claude_cwd_판정실패` 가 뜨면 그 문서를 본다.
