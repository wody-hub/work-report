...
`setup` 이 끝나면 **세션 로그를 스캔한 경로 후보가 바로 화면에 뜬다.** 거기서 골라 설정을 만든다.

```bash
~/.claude/skills/work-report/scripts/run.sh --init --write \
  "$HOME/Project/회사프로젝트" "$HOME/work-report-out" "me@company.com"
```

```
경로1:경로2…   수집 대상 (콜론 구분). 이 경로 하위에서 실행된 세션만 모은다
결과폴더        산출물을 둘 위치
이메일1:이메일2  git 커밋에서 내 것만 골라내는 author
```

첫 수집은 전체 기간이라 1~2분 걸린다. 이후는 하루치만 돌린다.

```bash
run.sh              # 전체 재생성 (처음 한 번)
run.sh --today      # 이후 일상 운영
run.sh --mark-uploaded   # 결과를 어딘가에 올린 뒤 실행
```

Claude Code 나 Codex 세션에서 `/work-report` 또는 "업무정리 해줘" 라고 해도 된다. 설정이 없으면 에이전트가 위 과정을 안내한다.

### 처음 쓰는 사람이 꼭 알아야 할 것

**1. 경로는 회사 것만 고른다.** 후보에 개인 프로젝트가 섞인 상위 경로가 같이 나온다.

```
    1248      73  ~/Project            ← 이걸 고르면 개인 작업까지 수집된다
     904      58  ~/Project/회사        ← 이걸 골라야 한다
     344      15  ~/Project/개인
```

**2. 결과 폴더를 `~/Documents`·`~/Desktop` 에 두지 않는다.** macOS iCloud 동기화 대상이라, 폴더를 다시 만들 때마다 ` 2` 가 붙은 충돌 폴더가 생기고 데이터가 애플 클라우드로 나간다.

**3. git author 이메일은 내 것만 고른다.** 후보에 팀원 이메일이 함께 나온다. 잘못 고르면 남의 커밋이 내 실적으로 들어간다. 여러 이메일로 커밋한 이력이 있으면 콜론으로 모두 나열한다.

**4. 산출물에는 자격증명이 남을 수 있다.** 토큰·API 키·비밀번호는 값을 `[REDACTED:…]` 로 마스킹하지만, 완벽하지 않다. 특히 **채팅창에 붙여넣은 서버 주소·계정명은 그대로 남는다.** 외부에 공유하기 전에 `run.sh` 마지막 단계의 민감정보 점검 결과를 확인한다. → [민감정보 점검](#민감정보-점검)

**5. 이 도구는 업로드하지 않는다.** 의도적이다. 결과 폴더에서 직접 올린다. 검토 없이 외부로 나가면 되돌릴 수 없다.

**6. 실적 숫자를 말할 때 `agent-tasks` 를 섞지 않는다.** AI 가 하위 에이전트에 넘긴 지시는 사람의 실적이 아니라 따로 분리돼 있다. 사람이 준 지시는 `instructions.*` 뿐이다.

문제가 생기면 [트러블슈팅](#트러블슈팅) 을 본다. 수집이 일부만 되는 것 같으면 실행 로그의 `커버리지` 줄과 `_reference/coverage*.json` 을 확인한다 → [수집 커버리지](#수집-커버리지--조용한-누락을-막는다)
...
11. [수집 커버리지](#수집-커버리지--조용한-누락을-막는다)
12. [집계 기준](#집계-기준)
13. [민감정보 점검](#민감정보-점검)
14. [업로드](#업로드)
15. [자동화](#자동화)
16. [업데이트와 제거](#업데이트와-제거)
17. [트러블슈팅](#트러블슈팅)
18. [FAQ](#faq)
...
> 포맷별 필드 위치와 새 포맷 대응 절차는 **[docs/log-formats.md](docs/log-formats.md)** 에 따로 정리했다. 도구를 고치거나 다른 사람에게 넘길 때 이 문서를 먼저 본다.
...
| `run.sh --date 2026-08-19` | 특정 날짜만. 콤마로 여러 날 지정 가능 |
| `run.sh --days 3` | 최근 3일만 |
...
`동일커밋 존재 저장소` 열은 포크·클론 관계로 같은 커밋이 다른 저장소에도 있을 때 채워진다. 커밋은 해시 기준으로 **한 번만** 센다 — 중복 제거하지 않으면 한 번 한 작업이 여러 번 계산된다.
...
실제 프로젝트 22개 저장소 · 커밋 3,086개 기준으로 **50MB / 361일**. 수집에 약 1분 40초 걸린다.

머지 아닌 커밋의 **89%** 가 patch 에 담긴다. 나머지는 변경 파일이 전부 제외 규칙에 걸린 커밋(아이콘·CSV·스키마 덤프)이다. 하루 상한을 넘는 커밋은 그 커밋만 건너뛰고 목록을 patch 끝에 남긴다 — 큰 커밋 하나 때문에 그날 나머지가 사라지지 않는다.

---

## 수집 커버리지 — 조용한 누락을 막는다

이 도구에서 가장 위험한 실패는 크래시가 아니라 **아무 말 없이 일부를 빼먹는 것**이다. 실제로 다음이 전부 조용히 일어나고 있었다.

| 누락 | 규모 | 원인 |
|---|---:|---|
| 구형 Codex 세션 통째로 skip | 35개 세션 / 지시 183건 | `session_meta` 없는 2025년 포맷 |
| 포크 저장소 커밋 중복 계산 | 1,943건 | 이력을 공유하는 저장소 |
| 큰 커밋이 그날 나머지를 날림 | 107일 / 370여 건 | 일별 상한에서 `break` |
| 접두어 중복제거로 지시 유실 | 698건 | 앞 400자만 비교 |

그래서 **매 실행마다 무엇을 못 담았는지 스스로 보고**한다.

```
[3/6] 코드 변경 수집
  361일 / 커밋 3,086개 / 49.7 MB
  커버리지: 머지 아닌 커밋 3,456개 중 3,086개 포함 (89%)
  미포함 사유: 변경 파일이 전부 제외 규칙 365, 하루 용량 상한 초과 3, 변경 파일 없음 2
```

세션 쪽에서 판정 불가·누락이 하나라도 생기면 노란색 `누락:` 줄이 뜬다. 0 이면 아무것도 출력하지 않는다.

기계로 읽을 수 있게 `_reference/` 에도 남긴다.

| 파일 | 내용 |
|---|---|
| `coverage.json` | Claude cwd 판정 실패, Codex 메타·cwd 누락, 대상 경로 밖 개수, 마스킹 유형별 건수 |
| `coverage-commits.json` | 저장소 수, 커밋 수, 포크 중복 제거 건수, 머지 수, author |
| `coverage-diffs.json` | 커버리지 %, 미포함 사유별 집계, **미포함 커밋 해시 전체 목록** |

미포함 커밋 해시가 남으므로, 연구소가 "이 커밋은 왜 코드가 없나" 를 되짚을 수 있고 필요하면 개별로 요청할 수 있다.

### 사유를 읽는 법

| 사유 | 조치 필요? |
|---|---|
| `변경 파일이 전부 제외 규칙` | 아니오. 아이콘·CSV 데이터·스키마 덤프만 바뀐 커밋 |
| `변경 파일 없음` | 아니오. 빈 커밋 |
| `하루 용량 상한 초과` | 필요하면 `WORK_CODE_MAX_DAY_KB` 를 올린다 |
| `저장소 못찾음` | **예.** 저장소를 옮겼거나 `WORK_TARGETS` 가 어긋났다 |
...
**파일을 지우는 건 답이 아니다.** 원본 세션 로그에 영구히 남아 있어서 재생성할 때마다 다시 나온다. 해당 자격증명을 폐기(토큰 회수, 키 로테이션, 비밀번호 변경)해야 그 문자열이 의미 없는 텍스트가 된다.

### 마스킹

탐지에서 그치지 않고 **값을 `[REDACTED:…]` 로 바꿔서** 출력한다 (기본 켜짐, `WORK_MASK=0` 으로 끔).

```
"sudo 권한은 !unecom1231! 이거 비밀번호야"
  → "sudo 권한은 [REDACTED:SECRET] 이거 비밀번호야"
...
**구조화된 토큰** — 형태가 고유해서 값까지 정확히 잡힌다. 개인키 블록 · SSH 공개키 · JWT · `ya29.…` · `1//…` · `sk-…` · `gh[pousr]_…` · `AKIA…` · `xox[abprs]-…` · `Bearer …` · `password=…` · `sshpass -p …`

**자유형 비밀번호** — `!unecom1231!` 같은 값은 형태가 없다. 토큰 모양으로 판정하되 오탐을 피하려고 기준을 둘로 나눈다.

| 상황 | 기준 |
|---|---|
| 자격증명 키워드 60자 이내 | 문자 + 기호 (`비밀번호`, `password`, `계정정보` 등) |
| 키워드 없음 | 문자 + 숫자 + `!` 를 모두 포함 |

키워드 없는 곳에서 `!` 를 요구하지 않으면 CSS 색상(`#34d399`), Java 프록시(`$Proxy110`), URL 파라미터(`1&size`)가 전부 걸린다. 실측 코퍼스 5,863건에서 이 기준으로 **고유 12개를 잡고 오탐 0** 이었다.

확실히 지우고 싶은 값은 `~/.config/work-report/secrets.txt` 에 한 줄씩 적으면 전 구간에서 치환된다 (`WORK_MASK_FILE`). 다만 그 파일 자체가 평문 비밀번호 목록이 되므로, 자격증명을 폐기하는 편이 낫다.

IP 마스킹은 기본 꺼져 있다 (`WORK_MASK_IP=1` 로 켬). 버전번호·좌표와 겹쳐 오탐이 많다.

### 마스킹 후에도 남는 것

마스킹은 **값**을 지우지 키워드나 문맥은 남긴다. 점검 단계는 마스킹 뒤에 돌기 때문에 `비밀번호`, `접속정보`, IP 같은 잔여 신호를 계속 보고한다. 그게 정상이다 — 사람이 마지막으로 확인하라는 뜻이다.

채팅창에 토큰이나 비밀번호를 붙여넣지 않는 게 최선이다.
